...
class SocialLoginAcceptanceTest {
    ...
    void 소셜_최초_가입시_연령대_직무가_없으면_400으로_거절된다() throws Exception {
        SocialLoginRequest request = new SocialLoginRequest("new1@test.com:테스트유저", null, null);
    ...
    void 소셜_최초_가입은_연령대_직무와_함께_성공하고_MEMBER_권한으로_발급된다() throws Exception {
        SocialLoginRequest request = new SocialLoginRequest("new2@test.com:테스트유저", AgeGroup.TWENTIES, Job.DEVELOPER);
    ...
    void 같은_소셜_계정으로_재로그인하면_새_계정이_생기지_않고_같은_회원으로_로그인된다() throws Exception {
        ...
        MvcResult first = mockMvc.perform(post("/api/auth/social/google")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new SocialLoginRequest(token, AgeGroup.TWENTIES, Job.DEVELOPER))))
                .andExpect(status().isOk())
                .andReturn();
        ...
        MvcResult second = mockMvc.perform(post("/api/auth/social/google")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new SocialLoginRequest(token, null, null))))
                .andExpect(status().isOk())
                .andReturn();
    ...
    void 이미_비밀번호로_가입된_이메일로_소셜_로그인하면_자동연동되지_않고_409로_거절된다() throws Exception {
        ...
        SocialLoginRequest request = new SocialLoginRequest("collide@test.com:소셜이름", AgeGroup.TWENTIES, Job.DEVELOPER);
        mockMvc.perform(post("/api/auth/social/google")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(request)))
    ...
    void 지원하지_않는_provider로_요청하면_400으로_거절된다() throws Exception {
        SocialLoginRequest request = new SocialLoginRequest("x@test.com:x", AgeGroup.TWENTIES, Job.DEVELOPER);
    ...
    void 로그인한_계정에_소셜_계정을_연동하면_이후_소셜_로그인으로_같은_계정에_들어온다() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/social/google/link")
                        ...
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new SocialLinkRequest(socialToken))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success").value(true));

        MvcResult socialLoginResult = mockMvc.perform(post("/api/auth/social/google")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new SocialLoginRequest(socialToken, null, null))))
                .andExpect(status().isOk())
                .andReturn();
    ...
    void 본인_이메일과_다른_소셜_계정을_연동하려_하면_400으로_거절된다() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/social/google/link")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new SocialLinkRequest("stranger@test.com:다른사람"))))
                .andExpect(status().isBadRequest())
    ...
    void 이미_연동된_소셜_계정을_다시_연동하려_하면_409로_거절된다() throws Exception {
        ...
        SocialLinkRequest linkRequest = new SocialLinkRequest(email + ":연동소셜이름");
    ...
    void 연동_엔드포인트는_인증_없이_호출하면_거절된다() throws Exception {
        mockMvc.perform(post("/api/auth/social/google/link")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new SocialLinkRequest("anyone@test.com:이름"))))
